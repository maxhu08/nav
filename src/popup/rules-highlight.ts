import { popupRuleEditorHighlightEl, popupRuleEditorTextareaEl } from "~/src/popup/ui";
...
export const syncPopupRuleHighlight = (): void => {
  let previousLineWasRuleStart = false;

  popupRuleEditorHighlightEl.innerHTML = popupRuleEditorTextareaEl.value
    .split("\n")
    .map((line) => {
      const renderedLine = renderLine(line, previousLineWasRuleStart);
      previousLineWasRuleStart = line.trim().startsWith("*");
      return renderedLine;
    })
    .join("\n");
};

export const syncPopupRuleHighlightScroll = (): void => {
  popupRuleEditorHighlightEl.scrollTop = popupRuleEditorTextareaEl.scrollTop;
  popupRuleEditorHighlightEl.scrollLeft = popupRuleEditorTextareaEl.scrollLeft;
};